...
async def phrasal_verbs(
    ...
    while True:
        try:
            ...
            if map_answer:
                mappedResult = _map(
                    improvedResult if improvedResult else _improve(responseResult)  # type: ignore[override]
                )
    ...
    value = _construct_answer(
        mappedResult=mappedResult,  # type: ignore[override]
        improvedResult=improvedResult,  # type: ignore[override]
        rawResult=responseResult,  # type: ignore[override]
...
def _queryChatGpt(context: Dict[str, Any]):
    if fake_response:
...
def _phrasal_verbs_demo_text(context: Dict[str, Any]):
    return {
        "result":
...
def _phrasal_verbs_demo_json(context: Dict[str, Any]):
    return {
        "result": {
